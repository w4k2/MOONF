...
def pairs_metrics_multi_line(method_names, data_np, dataset_names, metrics, filename, ref_methods, treshold=0.5):
    ...
    fig.suptitle(ref_methods[0], fontsize=24, x=0.5, y=1.6)
...
def pairs_metrics_to_table(method_names, data_np, dataset_names, metrics, filename, ref_methods, treshold=0.5):
    # Load data
    data = {}
    for dataset_id, dataset_name in enumerate(dataset_names):
        for method_id, method_name in enumerate(method_names):
    ...
    for index_j, metric in enumerate(metrics):
        ...
        # Save in table .tex
        # --------------------------------------
        if not os.path.exists("results/tables_wilcoxon/"):
            os.makedirs("results/tables_wilcoxon/")
        with open("results/tables_wilcoxon/wilcoxon_%s_%s.tex" % (metric, method_names[0]), "w+") as file:
            columns = ""
            for i in range(5):
                columns += " l"
            print("\\begin{tabular}{%s}" % columns, file=file)
            for method_id, method_name in enumerate(method_names[1:]):
                print("\\midrule", file=file)
                columns_names = f'{{{method_names[0]}}} & & '
                method_name_changed = method_name.replace("_", "-")
                columns_names += f'{{{method_name_changed}}} & & {{p-value}}\\\\'
                print(columns_names, file=file)
                wins_plus_half_ties = rank_win[method_id] + floor(0.5*rank_tie[method_id])
                lose_plus_half_ties = rank_lose[method_id] + floor(0.5*rank_tie[method_id])
                if wins_plus_half_ties > lose_plus_half_ties:
                    print("\\textbf{%d} & & %d & & %0.4f\\\\" % (wins_plus_half_ties, lose_plus_half_ties, p_values[method_name]), file=file)
                elif wins_plus_half_ties < lose_plus_half_ties:
                    print("%d & & \\textbf{%d} & & %0.4f\\\\" % (wins_plus_half_ties, lose_plus_half_ties, p_values[method_name]), file=file)
                else:
                    print("%d & & %d & & %0.4f\\\\" % (wins_plus_half_ties, lose_plus_half_ties, p_values[method_name]), file=file)
            print("\\midrule", file=file)
